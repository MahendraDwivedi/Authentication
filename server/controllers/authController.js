...
export const register = async(req,res)=>{
    ...
    try {
        ...
        const mailOptions = {
            ...
            text:`Welcome to Mahi website. Your account has been created with email id:${email}`
        }
...
export const sendVerifyOtp = async (req,res)=>{
    try {
        ...
        const mailOptions = {
            from:process.env.SENDER_EMAIL,
            to:user.email,
            subject:'Accoutnt Verification OTP',
...
export const sendResetOtp = async(req,res)=>{
    ...
    try {
        const user = await userModel.findOne({email});
        if(!user) return res.json({success:false,message:"User not found"});
        // if(user.isAccountVerified===false) return res.json({success:false,message:"Account not verified"});
        const otp = Math.floor(100000 + Math.random() * 900000);
        user.resetOtp = otp;
...
export const resetPassword = async(req,res)=>{
    ...
    try {
        ...
        // if(user.password===newPassword) return res.json({success:false,message:"New password should be different from old password"});
